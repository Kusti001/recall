...
export function CardsTable({ cards, selectedCardId, onSelectCard }: CardsTableProps) {
  return (
    <div className="overflow-hidden rounded-xl border">
      <div className="grid grid-cols-6 border-b bg-muted/30 px-4 py-3 text-xs text-muted-foreground">
        <span></span>
        <span>Лицо</span>
        <span>Оборот</span>
        <span>Интервал</span>
      ...
      {cards.map((card) => (
        <button
          ...
          className={`grid w-full grid-cols-6 items-center border-b px-4 py-3 text-left text-sm hover:bg-muted/50 ${
